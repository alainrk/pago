/*
Pago API

Personal finance tracking API. All /api/_* endpoints require either a session cookie (browser) or an Authorization: Bearer <token> header (machine clients).

API version: 1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package pago

import (
	"encoding/json"
)

// checks if the WebSearchTransactionsResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &WebSearchTransactionsResponse{}

// WebSearchTransactionsResponse struct for WebSearchTransactionsResponse
type WebSearchTransactionsResponse struct {
	Limit        *int32              `json:"limit,omitempty"`
	Offset       *int32              `json:"offset,omitempty"`
	Total        *int32              `json:"total,omitempty"`
	Transactions []WebTransactionDTO `json:"transactions,omitempty"`
}

// NewWebSearchTransactionsResponse instantiates a new WebSearchTransactionsResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWebSearchTransactionsResponse() *WebSearchTransactionsResponse {
	this := WebSearchTransactionsResponse{}
	return &this
}

// NewWebSearchTransactionsResponseWithDefaults instantiates a new WebSearchTransactionsResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWebSearchTransactionsResponseWithDefaults() *WebSearchTransactionsResponse {
	this := WebSearchTransactionsResponse{}
	return &this
}

// GetLimit returns the Limit field value if set, zero value otherwise.
func (o *WebSearchTransactionsResponse) GetLimit() int32 {
	if o == nil || IsNil(o.Limit) {
		var ret int32
		return ret
	}
	return *o.Limit
}

// GetLimitOk returns a tuple with the Limit field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebSearchTransactionsResponse) GetLimitOk() (*int32, bool) {
	if o == nil || IsNil(o.Limit) {
		return nil, false
	}
	return o.Limit, true
}

// HasLimit returns a boolean if a field has been set.
func (o *WebSearchTransactionsResponse) HasLimit() bool {
	if o != nil && !IsNil(o.Limit) {
		return true
	}

	return false
}

// SetLimit gets a reference to the given int32 and assigns it to the Limit field.
func (o *WebSearchTransactionsResponse) SetLimit(v int32) {
	o.Limit = &v
}

// GetOffset returns the Offset field value if set, zero value otherwise.
func (o *WebSearchTransactionsResponse) GetOffset() int32 {
	if o == nil || IsNil(o.Offset) {
		var ret int32
		return ret
	}
	return *o.Offset
}

// GetOffsetOk returns a tuple with the Offset field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebSearchTransactionsResponse) GetOffsetOk() (*int32, bool) {
	if o == nil || IsNil(o.Offset) {
		return nil, false
	}
	return o.Offset, true
}

// HasOffset returns a boolean if a field has been set.
func (o *WebSearchTransactionsResponse) HasOffset() bool {
	if o != nil && !IsNil(o.Offset) {
		return true
	}

	return false
}

// SetOffset gets a reference to the given int32 and assigns it to the Offset field.
func (o *WebSearchTransactionsResponse) SetOffset(v int32) {
	o.Offset = &v
}

// GetTotal returns the Total field value if set, zero value otherwise.
func (o *WebSearchTransactionsResponse) GetTotal() int32 {
	if o == nil || IsNil(o.Total) {
		var ret int32
		return ret
	}
	return *o.Total
}

// GetTotalOk returns a tuple with the Total field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebSearchTransactionsResponse) GetTotalOk() (*int32, bool) {
	if o == nil || IsNil(o.Total) {
		return nil, false
	}
	return o.Total, true
}

// HasTotal returns a boolean if a field has been set.
func (o *WebSearchTransactionsResponse) HasTotal() bool {
	if o != nil && !IsNil(o.Total) {
		return true
	}

	return false
}

// SetTotal gets a reference to the given int32 and assigns it to the Total field.
func (o *WebSearchTransactionsResponse) SetTotal(v int32) {
	o.Total = &v
}

// GetTransactions returns the Transactions field value if set, zero value otherwise.
func (o *WebSearchTransactionsResponse) GetTransactions() []WebTransactionDTO {
	if o == nil || IsNil(o.Transactions) {
		var ret []WebTransactionDTO
		return ret
	}
	return o.Transactions
}

// GetTransactionsOk returns a tuple with the Transactions field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebSearchTransactionsResponse) GetTransactionsOk() ([]WebTransactionDTO, bool) {
	if o == nil || IsNil(o.Transactions) {
		return nil, false
	}
	return o.Transactions, true
}

// HasTransactions returns a boolean if a field has been set.
func (o *WebSearchTransactionsResponse) HasTransactions() bool {
	if o != nil && !IsNil(o.Transactions) {
		return true
	}

	return false
}

// SetTransactions gets a reference to the given []WebTransactionDTO and assigns it to the Transactions field.
func (o *WebSearchTransactionsResponse) SetTransactions(v []WebTransactionDTO) {
	o.Transactions = v
}

func (o WebSearchTransactionsResponse) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o WebSearchTransactionsResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Limit) {
		toSerialize["limit"] = o.Limit
	}
	if !IsNil(o.Offset) {
		toSerialize["offset"] = o.Offset
	}
	if !IsNil(o.Total) {
		toSerialize["total"] = o.Total
	}
	if !IsNil(o.Transactions) {
		toSerialize["transactions"] = o.Transactions
	}
	return toSerialize, nil
}

type NullableWebSearchTransactionsResponse struct {
	value *WebSearchTransactionsResponse
	isSet bool
}

func (v NullableWebSearchTransactionsResponse) Get() *WebSearchTransactionsResponse {
	return v.value
}

func (v *NullableWebSearchTransactionsResponse) Set(val *WebSearchTransactionsResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableWebSearchTransactionsResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableWebSearchTransactionsResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWebSearchTransactionsResponse(val *WebSearchTransactionsResponse) *NullableWebSearchTransactionsResponse {
	return &NullableWebSearchTransactionsResponse{value: val, isSet: true}
}

func (v NullableWebSearchTransactionsResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWebSearchTransactionsResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
